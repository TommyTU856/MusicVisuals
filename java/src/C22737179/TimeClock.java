...
class TimeClock {
    ...
    public void drawTimeClock() {
        ...
        pa.rotate(-PConstants.HALF_PI); // Rotate coordinate system so that 0 is located at the top
    
        // Arc display for hour
        float H_a = PApplet.map(PApplet.hour(), 0, 23, 0, PConstants.TWO_PI);
        pa.noFill();
        ...
        float M_a = PApplet.map(PApplet.minute(), 0, 59, 0, PConstants.TWO_PI);
        ...
        float S_a = PApplet.map(PApplet.second(), 0, 59, 0, PConstants.TWO_PI);
        ...
        String time = PApplet.hour() + ":" + PApplet.minute() + ":" + PApplet.second(); // join time values into a string
